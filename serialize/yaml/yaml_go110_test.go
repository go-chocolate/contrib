...
func TestUnmarshalWithTags(t *testing.T) {
	...
	t.Run("Known tagged field", func(t *testing.T) {
		y := []byte(`field: "hello"`)
		v := WithTaggedField{}
		if err := UnmarshalOpts(y, &v, DisallowUnknownFields); err != nil {
	...
	t.Run("With unknown tagged field", func(t *testing.T) {
		...
		v := WithTaggedField{}
		err := UnmarshalOpts(y, &v, DisallowUnknownFields)
		if err == nil {
...
//		s := UnmarshalString{}
//		err := UnmarshalOpts(tc.yaml, &s, tc.opts...)
...
func ExampleUnknown() {
	...
	y := []byte(`unknown: "hello"`)
	v := WithTaggedField{}
	fmt.Printf("%v\n", UnmarshalOpts(y, &v, DisallowUnknownFields))
	// Ouptut:
